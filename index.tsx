...
export function MyButtonGrid() {
  const [visibleButtonId, setVisibleButtonId] = useState<number | null>(null);
  const [wigglingButtonId, setWigglingButtonId] = useState<number | null>(null);

  const handleClick = (id: number) => {
    setVisibleButtonId(visibleButtonId === id ? null : id); // Toggle visibility per button

    // Set the button to wiggle
    setWigglingButtonId(id);

    // Remove the wiggle effect after 500ms (same as animation duration)
  ...
  return (
    <div className="button-grid">
      {buttonData.map(({ id, imageSrc, text }) => (
        <div key={id}>
          {/* Button with background image and wiggle effect */}
          <button
          ...
          {/* Conditionally render the text with the 'show' class if this button is clicked */}
          <div
